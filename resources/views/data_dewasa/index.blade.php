...
<div class="card card-default">
    <div class="card-header">
        <form action="{{ url('dewasa') }}" method="GET" class="form-inline">
            ...
            <div class="form-group mr-1">
                <a class="btn-sm btn btn-primary" href="{{ route('dewasa.create') }}">Tambah</a>
            </div>
        </form> <br>
    ...
    <tr>
        ...
        @foreach ($dewasa as $product)
            <tr>
                <td>{{ $product->id }}</td>
                 <td>
                    <a href="{{ route('dewasa.show',$product->id) }}">{{ $product->nama_lengkap}}</a>
                </td>
                <td>{{ $product->no_telpon }}</td>
                <td>{{ $product->jenis }}</td>
                <td><img src="/gambar/{{ $product->gambar }}" width="100px"></td>
                <td>
                    <form action="{{ route('dewasa.destroy',$product->id) }}" method="POST">
                    <a class="btn btn-success btn-sm " href="{{ route('dewasa.edit',$product->id) }}"><i class="fas fa-edit"></i></a>
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger btn-sm "><i class="fa fa-trash"></i></button>
                    <a class="btn btn-default btn-sm " href="{{ route('dewasa.show',$product->id) }}"><span class="glyphicon glyphicon-download-alt"></span> PDF</a>
