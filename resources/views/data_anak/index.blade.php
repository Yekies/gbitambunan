...
<div class="card card-default">
    <div class="card-header">
        <form action="{{ url('anak') }}" method="GET" class="form-inline">
            ...
            <div class="form-group mr-1">
                <a class="btn-sm btn btn-primary" href="{{ route('anak.create') }}" >Tambah</a>
            </div>
        </form> <br>
    ...
    <tr>
        ...
        @foreach ($anak as $product)
            <tr>
                <td>{{ $product->id }}</td>
                 <td>
                    <a href="{{ route('anak.show',$product->id) }}">{{ $product->nama_lengkap}}</a>
                </td>
                <td>{{ $product->no_telpon }}</td>
                <td>{{ $product->jenis }}</td>
                <td><img src="/gambar/{{ $product->gambar }}" width="100px"></td>
                <td>
                    <form action="{{ route('anak.destroy',$product->id) }}" method="POST">
                    <a class="btn btn-success btn-sm " href="{{ route('anak.edit',$product->id) }}"><i class="fas fa-edit"></i></a>
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger btn-sm "><i class="fa fa-trash"></i></button>
                    <a class="btn btn-default btn-sm " href="{{ route('anak.show',$product->id) }}"><span class="glyphicon glyphicon-download-alt"></span> PDF</a>
